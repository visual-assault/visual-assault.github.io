...
date: 2016-03-07 20:38:00
metaDescription: "A guide for installing Arch Linux using Encryption on an Apple MacBook pro"
...
and GNU/Linux. I've tested this guide on both of the above generations, but I'm
assuming it'll work on anything in between. However, I make no guarantees.
...
going to be using OS X
at all, because right now (and likely for the foreseeable future) the only
way to get firmware updates installed on your machine will be via OS X.
...
Apple product, the 15" MacBook Pro 8,2− which I still use today (Feb 2016,
...
torq screws, they're fairly easy to repair or upgrade. Only recently had
...
changed their hardware designs to lock-down components. The
...
directly the logic board. Later models would have the hard drive soldered
down as well.
...
I first started using Linux in 1997-1998 when I bought a [shrink-wrapped box
of RedHat Linux 5.2](https://upload.wikimedia.org/wikipedia/en/thumb/4/49/Redhat_5.2_box.jpg/220px-Redhat_5.2_box.jpg).
At the time, my internet connection was fast enough
to download the ISO image over a 1-day dial-up session, but not reliable
enough. Since then I've tried Slackware, SuSE, Mandrake, Debian, Fedora,
Ubuntu, CentOS, and Mint. I customized the hell out of my distros, I performance
tweaked, secured and beautified the GUI. I did it all. Back then, information
...
1. The community package repository is great at providing pretty much
...
> Arch is like a sandbox, they provide the box and a source of sand (the
  core and community package repos), but  it's up to you to fill it and
  build your castle.
...
them as needed and for it to be fast. The content of the application is
the eye-candy, not the chrome around the edges of the window.

Awesome was designed for this.

So, this guide is going to install Awesome as it's window manager. Feel
free to switch it out for whatever you prefer.
...
that, ideally into the same directory as your Arch ISO. Then run the
...
source to download the ISO from.
...
After that’s run, it USB drive should be ready to boot from.
...
you created earlier, and substitute the `X` with the number of the drive you took note
of earlier.
...
drive and fail as it won't recognize the formatting. Click ignore or eject.
...
to put files into. Partitioning the hard drive takes existing data on the
...
into sub-partitions later; after the encryption is setup.
...
the ending sector of the adjacent partition.
...
**Note** This will just encrypt the system /root and /home directories. The
...
for the `/` and user directories. I gave the `/` directory 40GB (hopefully
enough for all my programs and whatnot. As of writing this, and with a full
...
We can now mount these new partitions.
Make sure to mount the root partition first so we can create the `/home`
directory inside of it for the home partition:
...
If you're interested in other partition schemes, [check this script
...
Delete or comment out all the servers except one or two in the US that seem
good.


### Install the Base System

Actually installing Arch Linux onto the drive is the easy part:

    pacstrap -i /mnt base base-devel terminus-font

The `-i` flag asks for confirmation before installing packages. I like
using it just so I can see what’s being installed. (After all that’s part of the reason
...
is now installed. Before changing into our installation, though, we need
...
**Note** If the `discard` option isn’t there, edit the fstab file and add
it *if you're hard drive is a solid-state drive (SSD)*. It's used for
...
The default font in the virtual console is not very readable, so lets setup
...
There are other sizes available: 12, 14, 16, 20, 22, 24, 28, 32.
...
boot-sequence to load our custom font and keyboard (USB too) so we can type
...
This will create the user, add him to the group wheel (traditional group
...
**Note**: On the options line, make sure your your drive replaces `/dev/sda3`
...
[enable it here](http://loicpefferkorn.net/2015/01/arch-linux-on-macbook-pro-retina-2014-with-dm-crypt-lvm-and-suspend-to-disk/#suspend-to-disk-uswsusp:385892e3ac2613dca78d22bd09dbae7d).

Now we can have the bootloader write the initramfs onto the boot
partition using `bootctl`:
...
You should now be running Arch Linux! Lets customize it and make it useful.
...
we'll install a utiliity named `yaourt`. You can (of course) maintain AUR
...
to control the speakers. Select the NVidia soundcard. Make sure to disable
channels for speakers you don't have. Test your speakers with
...
driver, the Mesa OpenGL graphics library, and video acceleration API for Intel.
It’ll likely ask you to install additional dependencies. Get all the
dependencies!
...
I like to install the utilities, too, because there are at least a couple I’ll
use later (either in this guide or another) that are helpful in improving HiDPI
support for the MacBook’s HiDPI Retina display.


### Some GUI Applications

Next we'll install a browser. This will install some typefaces and font-
rendering libraries that we'll be tweaking later.
...
Infinality is a package group that has been
meticulously crafted from the ground-up to provide beautifully rendered
typography to the Linux platform. The package maintainer spent a ton of
time tearing down the existing font stack, and carefully rebuilding it
to provide fast-rendering fonts that more faithfully present the typography
than either Windows, OS X or Ubuntu systems. It also provides a set of
preset-configurations that allow the user to switch to a Windows-like,
OS X-like configuration. I highly recommend [reading the
documentation](http://bohoomil.com/doc/) if you'd like to know more about
typography.
...
To enable selection of predefined font substitution styles and antialiasing settings,
apart from the rendering settings of the engine itself. After doing so, you
can select the font style (win7, winxp, osx, linux, ...) with:
...
The following config at /etc/X11/xorg.conf.d/50-synaptics.conf works well for
me, it uses the same "Natural Motion" that OS X does.
...
greeter as well as a KDE greeter, both option dependencies. I've skipped
over those in favor of the `webkit2` greeter. This uses the HTML
...
you can [view it on the web](https://fiddle.jshell.net/0xADADA/nqynn75j/show/light/)
...
Now lets get LightDM to start right after the system boots up.
...
your new Arch Linux box with Cinnamon.

## Fine Tuning

Another thing I’m often trying to get just right are the power settings. I’ve
seen several things online and have tried many of the suggestions. Some worked,
some didn’t. Here’s what I ended up using.

I should note that this still isn’t perfect. My MacBook definitely runs cooler
now, and battery life seems a bit better. I usually get about 6 hours on a
charge. On OS X I got about 8, so it’s not too bad. Still room for improvement,
though.

Suspend functionality still needs some work, but thats for another day.
...
“Yosemite” then you should be OK.
...
Second, install the `xcalib` (AUR) package.
...
the `.xinitrc` file. Just make sure you replace `FILENAME` with the name
of your color-profile `.icc` file exported from OS X.
...
The power management from Arch out of the box is decent, but not very good.
...
You can create a systemd service that will start powertop’s autotune
settings on startup.
...
information using the command `acpi -i`. To install:
...
maxing out at all times. My `cpupower`
config file at `/etc/default/cpupower` has the following two lines
uncommented:
...
bind keyboard keys to special commands. We will bind keys for keyboard and
display brightness as well as volume.
